...
@WebSocketGateway({ namespace: '/platform-esp', pingTimeout: 5000, pingInterval: 1000 })
export class EspGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleConnection(client: Socket, ...args: any[]) {
    ...
    setTimeout(() => { Notify.unAuthorized(client) }, 1000)
  }
  ...
  handleAuth(client: Socket, payload: any) {
    ...
    if (!EspGateway.isEspID(payload.id))
  ...
  handleSyncDetail(client: Socket, payload: any) {
    ...
    const oldSignal = NetworkUtil.calculateSignalLevel(oldDetail['detail']['data']['rssi'])
    const newSignal = NetworkUtil.calculateSignalLevel(newDetail['detail']['data']['rssi'])
...
class Pass {
  static def(objSrc: Object, objDest: Object): Object {
    if (isUndefined(objSrc))
      return {}

    if (isUndefined(objDest) || isNull(objDest))
      objDest = {}

    Object.keys(objSrc).forEach(key => {
      if (isUndefined(objDest[key]))
